...
function notify:ClearTop(player)
	notify:RemoveTop(player, 50)
end
...
function notify:ClearTopFromAll()
	notify:RemoveTopFromAll(50)
end
...
function notify:RemoveTop(player, count)
...
function notify:RemoveTopFromAll(count)
...
function notify:Bottom(player, table)
	...
	if table.text ~= nil then
		CustomGameEventManager:Send_ServerToPlayer(player, "bottom_notification", {text=table.text, duration=table.duration, class=table.class, style=table.style, continue=table.continue,error=table.error} )
	elseif table.hero ~= nil then
...
function notify:BottomToAll(table)
	if table.text ~= nil then
		CustomGameEventManager:Send_ServerToAllClients("bottom_notification", {text=table.text, duration=table.duration, class=table.class, style=table.style, continue=table.continue,error=table.error} )
	elseif table.hero ~= nil then
