...
--@param #number radius 范围（全屏使用FIND\_UNITS\_EVERYWHERE）
...
--DOTA_UNIT_TARGET_TREE
--</pre>
function FindAlliesInRadiusEx(team,point,radius,flag,order,targetType)
	if type(team) ~= "number" and type(team.GetTeamNumber) == "function" then
		team = team:GetTeamNumber()
	end
	flag = flag or DOTA_UNIT_TARGET_FLAG_NONE;
...
function SendToClient(player,eventName,data)
	...
	elseif type(player) == "table" then
		player = PlayerUtil.GetOwner(player)
	end
	if player then
...
function GetAbilitySpecialValueByLevel(ability,svName,level)
	if type(level) == "number" then
		return ability:GetLevelSpecialValueFor(svName,level - 1)
	else
		return ability:GetSpecialValueFor(svName)
	end
end
...
--@param #handle target 被击退目标单位
--@param #handle ability 击退使用的技能
--@param #number duration 击退持续时间
--@param #number distance 击退距离
--@param #number height 击退高度
--@param #boolean shouldStun 击退期间是否眩晕目标
--@param #boolean cooldown 冷却时间。如果不为空，则在冷却时间内，单位不会再次受到这个函数造成的击退效果
function KnockBackX(caster,target,ability,duration,distance,height,shouldStun,cooldown)
	if cooldown and target:FindModifierByName("td_modifier_knockback_interval") then
		return
	end
	
	local kv = {
		duration = duration,
		should_stun = shouldStun,
	...
	target:AddNewModifier(caster,ability, "modifier_knockback", kv )
	--击退冷却
	if cooldown then
		target:AddNewModifier(caster,nil, "td_modifier_knockback_interval", {duration=cooldown} )
	end
end
...
---将一个单位设置成幻象。并根据source同步等级、技能和物品
--@param #handel unit 要变成幻象的单位
--@param #handle source 幻象本体
--@param #handle ability 创造幻象的技能
--@param #number duration 持续时间，没有持续时间，将不添加幻象buff（显示剩余时间、伤害调整等）
--@param #number outgoingDamage 造成本体伤害百分比
--@param #number incomingDamage 受到本体伤害百分比
--@param #boolean needSync 是否需要同步幻象和本体的等级、技能和物品
function MakeUnitIllusion(unit,source,ability,duration,outgoingDamage,incomingDamage,needSync)
	local casterAngle = source:GetAngles()
	unit:SetAngles(casterAngle.x,casterAngle.y,casterAngle.z)
	--等级
	if unit.CreatureLevelUp then
		unit:CreatureLevelUp(source:GetLevel() - 1)
	elseif unit.HeroLevelUp then
		local casterLevel = source:GetLevel()
		for i=1,casterLevel-1 do
			unit:HeroLevelUp(false)
		end
	end
	--技能。所有被动类的buff，都会被加到幻象身上，即时该buff在kv中指明了幻象不可继承。。。
	...
	for index=0, 11 do
		...
		if ability then
			--不能直接获取同序号的，因为本体可能有额外添加的技能，如果直接通过序号获取，可能和幻象的技能不一致
			local originAbility = source:FindAbilityByName(ability:GetAbilityName())
			if originAbility then
				ability:SetLevel(originAbility:GetLevel())
			end
		end
	end
	--物品
	for itemSlot=0,8 do
		local item = source:GetItemInSlot(itemSlot)
		if item ~= nil then
	...
	--这两个都是额外的系数，所以要减去100
	if type(outgoingDamage) == "number" then
		outgoingDamage = outgoingDamage - 100
	end
	if type(incomingDamage) == "number" then
		incomingDamage = incomingDamage - 100
	end
	
	--加一个幻象的图标，还有持续时间（在幻象左下角显示的倒计时），还有伤害变化等。
	unit:AddNewModifier(source, ability, "modifier_illusion", { duration = duration, outgoing_damage = outgoingDamage, incoming_damage = incomingDamage })
	--使单位成为幻象（IsIllusion返回true）：物品和技能会被禁用，等等
	unit:MakeIllusion()
	
	--定时同步幻象属性
	if needSync then
		TimerUtil.createTimer(function()
			if not EntityIsAlive(unit) or not EntityIsAlive(source) then
				return;
			end
			
			--等级
			local differ = source:GetLevel() - unit:GetLevel()
			if differ > 0 then
				if unit.CreatureLevelUp then
					unit:CreatureLevelUp(differ)
				elseif unit.HeroLevelUp then
					for i=1,differ do
						unit:HeroLevelUp(false)
					end
			...
			--只同步了9个技能。一般最多四五个技能，9个应该就够了
			for index=0, 8 do
				local ability = unit:GetAbilityByIndex(index)
				if ability then
					--不能直接获取同序号的，因为本体可能有额外添加的技能，如果直接通过序号获取，可能和幻象的技能不一致
					local originAbility = source:FindAbilityByName(ability:GetAbilityName())
					if originAbility and ability:GetLevel() ~= originAbility:GetLevel() then
						ability:SetLevel(originAbility:GetLevel())
					end
				end
			end
			--物品，比较复杂，涉及到物品位置的交换等。其实简单点可以直接全部删除掉，然后再重新添加一遍。但是怕创建太多实体，影响效率
			local existItems = {}
			for slot=0, 8 do
				local item = unit:GetItemInSlot(slot)
				if item then
					local itemName = item:GetName()
					local items = existItems[itemName]
					if not items then
						items = {}
						existItems[itemName] = items
					end
			...
			for slot=0,8 do
				...
				if item ~= nil then
					local itemName = item:GetName()
					local newItem = existItems[itemName]
					if not newItem then
						newItem = CreateItem(itemName, unit, unit)
					else
						newItem = table.remove(existItems[itemName])
						if not newItem then
							existItems[itemName] = nil
							newItem = CreateItem(itemName, unit, unit)
						end
					end
					newItem.slot = slot
			...
			for key, items in pairs(existItems) do
				if #items > 0 then
			...
			---交换物品位置，后面的slot对应的期望slot也比较大（至少不会小于其实际位置），倒序交换位置，不会出现被遗漏的情况
			--必须交换位置，因为物品可以放在背包中，放在背包就没有效果了，如果不交换位置的话幻象和本体的属性就可能不一致，因为在本体身上
			--是存在于背包，而幻象却可能放在了物品栏。
			for slot=8, 0,-1 do
				local item = unit:GetItemInSlot(slot)
				if item ~= nil and item.slot and item.slot ~= slot then
					unit:SwapItems(slot,item.slot)
				end
			end
...
---创建一个幻象单位，和caster一样的朝向，等级、技能、物品复制caster。
--同时移除掉单位身上所有非幻象可继承的buff
--@param #handle caster 幻象本体
--@param #Vector pos 幻象出生点
--@param #handle ability 创造幻象的技能
--@param #number duration 持续时间
--@param #number outgoingDamage 造成本体伤害百分比
--@param #number incomingDamage 受到本体伤害百分比
function CreateIllusion(caster,pos,ability,duration,outgoingDamage,incomingDamage)
	local illusion = CreateUnitEX(caster:GetUnitName(), pos, false, caster, caster:GetTeamNumber())
	MakeUnitIllusion(illusion,caster,ability,duration,outgoingDamage,incomingDamage)
	return illusion
end
...
---获取游戏难度（没有设置的时候，返回0）
function GetTDDifficulty()
	return GameRules:GetCustomGameDifficulty()
end

---获取游戏模式。返回一个数字，1代表团队模式；2代表单人模式
function GetTDGameMode()
	return GetMapName() == "xxj" and 1 or 2;
end
...
---单位死亡监听处理
--@param #handle unit 要监听死亡的单位，该单位死亡的时候触发func函数
--@param #function func 单位死亡的处理逻辑，会传入以下参数：attacker，inflictor（被什么击杀的，可能是技能，可能为空）
--@param #handle caster 施法者，可选
--@param #handle ability 技能，可选
function DealWithUnitDeath(unit,func,caster,ability)
	if type(unit) == "table" and type(func) == "function" then
		local modifier = AddLuaModifier(caster,unit,"td_model_death_checker",{},ability)
		if modifier then
			--在初始化参数里面加了不好使，很蛋疼
			modifier.func = func
		end
...
---临时修改单位的模型和攻击音效
--@param #handle unit 单位实体
--@param #string modelName 新模型路径
--@param #number duration 持续时间，可为空，为空的时候，持续到单位死亡
--@param #string attackSoundName 攻击音效声音，可为空
--@param #boolean permanent 死亡不移除
function ChangeModelTemporary(unit,modelName,duration,attackSoundName,permanent)
	if type(unit) == "table" and type(modelName) == "string" then
		AddLuaModifier(nil,unit,"td_model_change",{duration=duration,model=modelName,sound=attackSoundName,permanent=permanent})
	end
end
