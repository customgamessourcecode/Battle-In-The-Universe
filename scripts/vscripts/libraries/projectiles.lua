...
function Projectiles:CreateTimer(name, args)
	if not args.endTime or not args.callback then
		DebugPrint("Invalid timer created: "..name)
		return
	end
...
--	fStartRadius = number,
--	fEndRadius = number,
--	iPositionCP = number, --特效中的刷新点控制点
--	iVelocityCP = number, --特效中的速度控制点
--	fExpireTime = number,
--	ControlPoints = {}, ---各个控制点的数据
--	UnitBehavior = PROJECTILES_XXX, --碰到单位后怎么处理特效
--	UnitTest = function() end
--	OnUnitHit = function() end
--	
--}
  ...
  elseif projectile.bDynamic == nil then
	...
	function projectile:SetVelocity(newVel, newPos)
		if projectile.changes > 0 then
			...
			--DebugPrint ('setting CP' .. projectile.iVelocityCP .. 'to ' .. tostring(newVel))
	...
	Projectiles:CreateTimer(projectile.ProjectileTimerName, {
		...
		callback = function()
			...
			--DebugPrint(tostring(framehalf) .. ' -- ' .. tostring(pos) .. ' -- ' .. (framehalf-pos):Length() .. ' -- ' .. radius)
			...
			for index=1,tot do
				...
				--DebugPrint(tostring(ground.z) .. ' -- ' .. tostring(pos.z))
				...
				if projectile.GroundBehavior ~= PROJECTILES_NOTHING and groundConnect then
					...
					elseif projectile.GroundBehavior == PROJECTILES_FOLLOW and projectile.changes > 0 and curTime >= projectile.changeTime then
						...
						--DebugPrint('follow ' .. slope:Dot(vel:Normalized()) .. ' -- ' .. projectile.changes)
