...
---组合塔伤害加深效果（有些世界中一些塔可以形成组合，用这个效果）<p>
--有组合的使用一个默认的modifier执行定时器检查，调用这个逻辑<br/>
--keys有:<br/>
--units(name1,name2,name3...)，组合中所有单位的名字
--level，组合中所有单位需要达到的等级
--modifier，满足要求后会给单位添加这个buff，主要用来提示加深数值，可有可无（没有的话就不会有buff提示，但是属性界面会显示增加的值）
function group_dmage_out_bonus(keys)
	local ability = keys.ability
	local caster = keys.caster
	...
	if type(units) == "string" and units ~= "" and type(level) == "number" then
		local names = Split(units,",")
		...
		if validCount == #names then
			if modifier then
				AddDataDrivenModifier(ability,caster,caster,modifier,{})
			end
			AddLuaModifier(caster,caster,"modifier_group_bonus_damage_out",{},ability)
		else
			if modifier then
				caster:RemoveModifierByNameAndCaster(modifier,caster)
			end
			caster:RemoveModifierByNameAndCaster("modifier_group_bonus_damage_out",caster)
		end
...
---增加受到的某系元素伤害。kv键值中必须有ratio，代表加深的系数。同名技能重复添加的时候，不会叠加，但是会刷新持续时间
--keys:
--element:元素类型，参考Elements，非空
--duration:持续时间，可为空。如果为空，则要用KVRemoveElementDamageIn去移除才可以正常移除加深效果
--key，唯一标识，不能为空，同标识的效果不同时存在，删除的时候也会用到（如果默认用ability去获取技能名字，在删除的时候会找不到ability而出错），一般写技能名字即可。
function KVAddElementDamageIn(keys)
	local ability = keys.ability
	local caster = keys.caster
	local target = keys.target
	local uniqueKey = keys.key
	...
	local element = keys.element
	local duration = keys.duration
	
	if element then
		--将添加成功的信息缓存在单位身上，方便移除。缓存在单位身上也最稳定，只要单位不死，就肯定会获取到。如果单位死了，那获取不到也无所谓了
		local key = "edo_"..uniqueKey
		if EntityIsNull(target[key]) then
			target[key] = Elements.AddElementDamageIn(caster,target,ability,element,duration)
		elseif duration then--已经存在的，刷新持续时间
			target[key]:SetDuration(duration,true)
		end
	end
...
---移除受到的某系元素伤害。对应KVAddElementDamageIn。如果直接调用，没有效果，必须在成功调用KVAddElementDamageIn后有意义
function KVRemoveElementDamageIn(keys)
	local ability = keys.ability
	local caster = keys.caster
	local target = keys.target
	local uniqueKey = keys.key
...
---增加某系元素伤害。kv键值中必须有ratio，代表加深的系数
--keys:
--element:元素类型，参考Elements，非空。只会给同类型的目标单位增加，如果目标单位的攻击类型和这个类型不匹配，则技能不进入冷却
--duration:持续时间，可为空。如果为空，则要用KVRemoveElementDamageOut去移除才可以正常移除加深效果
--tooltip:用于提示的buff，同时用来判断是否已经有增伤效果了，有的话就不添加了，只刷新
--key，唯一标识，不能为空，同标识的效果不同时存在，删除的时候也会用到（如果默认用ability去获取技能名字，在删除的时候会找不到ability而出错），一般写技能名字即可。
function KVAddElementDamageOut(keys)
	local ability = keys.ability
	local caster = keys.caster
	local target = keys.target
	local uniqueKey = keys.key
	...
	---如果没有类型或者有类型并且类型匹配，则添加增伤效果；否则只添加提示buff
	if not element or element == Elements.GetAttackElement(target) then
		--将添加成功的信息缓存在单位身上，方便移除。缓存在单位身上也最稳定，只要单位不死，就肯定会获取到。如果单位死了，那获取不到也无所谓了
		local key = "edo_"..uniqueKey
		if EntityIsNull(target[key]) then
			target[key] = AddLuaModifier(caster,target,"modifier_multiple_damage_out",{duration=duration},ability)
	...
	AddDataDrivenModifier(ability,caster,target,tooltip,{duration=duration})
end

---移除某系元素增伤
function KVRemoveElementDamageOut(keys)
	local ability = keys.ability
	local caster = keys.caster
	local target = keys.target
	local uniqueKey = keys.key
...
---给予金币，无尽模式和分裂箭类的攻击不会增加金币
--keys:gold 金币数量
--target:不指名默认是caster，如果指明了TARGET的话，则增加给目标单位所属玩家
--modifier:某些人物升级的时候会提升自身技能获取金币的数量。
--一般都用的是一个modifier_card_lvlup_bonus_money，有些特殊人物会有个性化的逻辑
--此时就用的是个性化的modifier，写上这个，就会去获取这个modifier而不是用默认的
function give_money(keys)
	---在无尽模式中，任何技能都不能获得金币
	...
	if gold then
		...
		--检查施法者是否经过了卡牌等级提升的强化
		local bonusModifier = nil
		if keys.modifier then
			bonusModifier = caster:FindModifierByName(keys.modifier);
		else
			bonusModifier = caster:FindModifierByName("modifier_card_lvlup_bonus_money");
		end
		if EntityNotNull(bonusModifier) and bonusModifier:GetStackCount() > 0 then
		...
		if keys.Target then
			...
			--分裂箭特殊处理
			local SplitAttackAbility = caster._SplitAttackAbility
			if SplitAttackAbility then
				local count = GetAbilitySpecialValueByLevel(SplitAttackAbility,"count")
				if count > 0 then
					gold = gold / count
				end
			end
	...
	elseif keys.target then
		...
		elseif not target.TD_IsBoss and not target.TD_IsSpecial then
			...
			if not keys.hideHint then
				NotifyUtil.ShowError(PlayerUtil.GetOwnerID(caster),"#error_can_not_use_on_unit",3)
			end
